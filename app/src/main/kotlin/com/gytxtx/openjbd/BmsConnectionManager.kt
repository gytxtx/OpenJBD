...
internal class BmsConnectionManager private constructor(private val context: Context) {
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(bluetoothGatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (status != BluetoothGatt.GATT_SUCCESS && newState != BluetoothProfile.STATE_DISCONNECTED) {
                failConnection(
                    BmsStateStore.ConnectionState.CONNECTION_FAILED,
                    getString(R.string.status_connection_failed, status)
                )
                return
            }
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                if (!hasConnectPermission()) {
                    failConnection(
                        BmsStateStore.ConnectionState.PERMISSION_REQUIRED,
                        getString(R.string.status_auto_connect_permission_required)
                    )
                    return
                }
                connected = true
                reconnectAttempts = 0
                updateState(
                    BmsStateStore.getSnapshot().withConnectionState(
                        BmsStateStore.ConnectionState.DISCOVERING_SERVICES,
                        true,
                        getString(R.string.status_connected_discovering)
                    )
                )
                if (!bluetoothGatt.discoverServices()) {
                    failConnection(
                        BmsStateStore.ConnectionState.SERVICE_DISCOVERY_FAILED,
                        getString(R.string.status_service_discovery_start_failed)
                    )
                }
                ...
                if (!intentionalDisconnect && shouldAutoReconnect()) {
                    ...
                    publishDisconnected(
                        BmsStateStore.ConnectionState.CONNECTION_FAILED,
                        getString(R.string.status_connection_lost),
                        true
                    )
                } else {
                    publishDisconnected(
                        BmsStateStore.ConnectionState.DISCONNECTED,
                        getString(R.string.status_select_bms)
                    )
                }
            }
        }
        ...
        override fun onServicesDiscovered(bluetoothGatt: BluetoothGatt, status: Int) {
            if (status != BluetoothGatt.GATT_SUCCESS) {
                failConnection(
                    BmsStateStore.ConnectionState.SERVICE_DISCOVERY_FAILED,
                    getString(R.string.status_service_discovery_failed, status)
                )
                return
            }
            ...
            if (service == null) {
                failConnection(
                    BmsStateStore.ConnectionState.SERVICE_NOT_FOUND,
                    getString(R.string.status_jbd_service_not_found)
                )
                return
            }
            ...
            if (writeCharacteristic == null || notifyCharacteristic == null) {
                failConnection(
                    BmsStateStore.ConnectionState.CHARACTERISTICS_NOT_FOUND,
                    getString(R.string.status_jbd_characteristics_not_found)
                )
                return
            }
            updateState(
                BmsStateStore.getSnapshot().withConnectionState(
                    BmsStateStore.ConnectionState.ENABLING_NOTIFICATIONS,
                    true,
                    getString(R.string.status_enabling_notifications)
                )
            )
            if (!enableNotifications(bluetoothGatt, notifyCharacteristic)) {
                failConnection(
                    BmsStateStore.ConnectionState.NOTIFICATIONS_FAILED,
                    getString(R.string.status_notifications_failed)
                )
            }
        }
    ...
    fun setAutoReconnect(enabled: Boolean, address: String?, name: String?) {
        ...
        autoReconnectName = if (name.isNullOrEmpty()) address else name
        if (!enabled) {
    ...
    private fun connectInternal(address: String?, name: String?, reconnect: Boolean) {
        ...
        if (adapter == null) {
            handleConnectPrecheckFailure(
                BmsStateStore.ConnectionState.BLUETOOTH_UNAVAILABLE,
                getString(R.string.status_bluetooth_unavailable),
                reconnect
            )
            return
        }
        ...
        if (!adapter!!.isEnabled) {
            handleConnectPrecheckFailure(
                BmsStateStore.ConnectionState.BLUETOOTH_OFF,
                getString(
                    if (reconnect) R.string.status_auto_connect_bluetooth_off else R.string.status_bluetooth_off
                ),
                reconnect
            )
            return
        ...
        connectedDeviceName = if (name.isNullOrEmpty()) address else name
        updateState(
            BmsStateStore.Snapshot.withConnectionState(
                BmsStateStore.ConnectionState.CONNECTING,
                false,
                connectedDeviceName,
                connectedDeviceAddress,
                getString(R.string.status_connecting, connectedDeviceName),
                null,
                null
            )
        )
        try {
            ...
            publishDisconnected(
                BmsStateStore.ConnectionState.INVALID_DEVICE,
                getString(R.string.status_auto_connect_invalid_device),
                true
            )
        }
    }
    ...
    fun cancelReconnect() {
        ...
        publishDisconnected(
            BmsStateStore.ConnectionState.DISCONNECTED,
            getString(R.string.status_reconnect_cancelled)
        )
    }
    ...
    @SuppressLint("MissingPermission")
    private fun closeGattIfPermitted(bluetoothGatt: BluetoothGatt) {
        if (hasConnectPermission()) {
            bluetoothGatt.close()
        }
    }
    ...
    private fun startReading() {
        updateState(
            BmsStateStore.getSnapshot().withConnectionState(
                BmsStateStore.ConnectionState.READY,
                true,
                getString(R.string.status_ready_reading)
            )
        )
        handler.removeCallbacks(pollRunnable)
    ...
    private fun enqueuePollCycle() {
        ...
        if (!extendedInfoRequested) {
            ...
            commandQueue.addLast(
                CommandRequest(
                    JbdCommands.CMD_FACTORY_MODE,
                    CommandKind.FACTORY_MODE,
            ...
            commandQueue.addLast(
                CommandRequest(
                    JbdCommands.CMD_EXTENDED_PARAMS,
                    CommandKind.EXT_RATINGS,
                    JbdCommands.readExtendedParams(EXT_RATINGS_START, 8)
                )
            )
            commandQueue.addLast(
                CommandRequest(
                    JbdCommands.CMD_EXTENDED_PARAMS,
                    CommandKind.EXT_BMS_ADDRESS,
                    JbdCommands.readExtendedParams(EXT_BMS_ADDRESS_START, 6)
                )
            )
            commandQueue.addLast(
                CommandRequest(
                    JbdCommands.CMD_EXTENDED_PARAMS,
                    CommandKind.EXT_BMS_MODEL,
                    JbdCommands.readExtendedParams(EXT_BMS_MODEL_START, 8)
                )
            )
            commandQueue.addLast(
                CommandRequest(
                    JbdCommands.CMD_CLOSE_FACTORY_MODE,
                    CommandKind.CLOSE_FACTORY_MODE,
    ...
    private fun drainCommandQueue() {
        ...
        val request = commandQueue.pollFirst()
        if (request == null) {
            handler.removeCallbacks(pollRunnable)
    ...
    private fun handleIncoming(value: ByteArray) {
        ...
        handler.post {
            for (frame in frames) {
                try {
                    if (frame.command == (JbdCommands.CMD_BASIC_INFO.toInt() and 0xFF)) {
                        val info = JbdParser.parseBasicInfo(frame)
                        val current = BmsStateStore.getSnapshot()
                        updateState(current.withBasicInfo(info, getString(R.string.status_ready_reading)))
                    } else if (frame.command == (JbdCommands.CMD_CELL_VOLTAGE.toInt() and 0xFF)) {
                    ...
                    val current = BmsStateStore.getSnapshot()
                    updateState(
                        current.withConnectionState(
                            BmsStateStore.ConnectionState.PARSE_ERROR,
                            connected,
    ...
    private fun failConnection(state: BmsStateStore.ConnectionState, message: String) {
        disconnectInternal(false)
    ...
    private fun publishDisconnected(state: BmsStateStore.ConnectionState, status: String) {
        publishDisconnected(state, status, false)
    }

    private fun publishDisconnected(state: BmsStateStore.ConnectionState, status: String, keepDevice: Boolean) {
    ...
    private fun handleConnectPrecheckFailure(
        state: BmsStateStore.ConnectionState,
        status: String,
        reconnect: Boolean
    ) {
    ...
    private fun scheduleReconnect(reason: String) {
        ...
        updateState(
            BmsStateStore.Snapshot.withConnectionState(
                ...
                connectedDeviceName,
                connectedDeviceAddress,
                getString(R.string.status_reconnecting, reason, delayMs / 1000L),
                null,
                null
            )
        )
    ...
    private fun getString(resId: Int, vararg args: Any?): String =
        AppSettings.preferredContext(context).getString(resId, *args)

    private fun localizedStatus(snapshot: BmsStateStore.Snapshot): String = when (snapshot.connectionState) {
        BmsStateStore.ConnectionState.DISCONNECTED -> getString(R.string.status_select_bms)
        BmsStateStore.ConnectionState.CONNECTING -> {
            if (!snapshot.deviceName.isNullOrEmpty()) {
                getString(R.string.status_connecting, snapshot.deviceName)
            } else {
                snapshot.status ?: getString(R.string.status_connecting, "")
            }
        }
        BmsStateStore.ConnectionState.DISCOVERING_SERVICES -> getString(R.string.status_connected_discovering)
        BmsStateStore.ConnectionState.ENABLING_NOTIFICATIONS -> getString(R.string.status_enabling_notifications)
        BmsStateStore.ConnectionState.READING, BmsStateStore.ConnectionState.READY ->
            getString(R.string.status_ready_reading)
        BmsStateStore.ConnectionState.BLUETOOTH_UNAVAILABLE -> getString(R.string.status_bluetooth_unavailable)
        BmsStateStore.ConnectionState.BLUETOOTH_OFF -> getString(R.string.status_bluetooth_off)
        BmsStateStore.ConnectionState.PERMISSION_REQUIRED ->
            getString(R.string.status_auto_connect_permission_required)
        BmsStateStore.ConnectionState.INVALID_DEVICE -> getString(R.string.status_auto_connect_invalid_device)
        BmsStateStore.ConnectionState.SERVICE_DISCOVERY_FAILED ->
            getString(R.string.status_service_discovery_start_failed)
        BmsStateStore.ConnectionState.SERVICE_NOT_FOUND -> getString(R.string.status_jbd_service_not_found)
        BmsStateStore.ConnectionState.CHARACTERISTICS_NOT_FOUND ->
            getString(R.string.status_jbd_characteristics_not_found)
        BmsStateStore.ConnectionState.NOTIFICATIONS_FAILED -> getString(R.string.status_notifications_failed)
    ...
    private class CommandRequest(
        ...
        companion object {
            fun read(command: Byte, kind: CommandKind): CommandRequest =
    ...
    companion object {
        ...
        @Volatile
        private var instance: BmsConnectionManager? = null

        @JvmStatic
        @Synchronized
        fun getInstance(context: Context): BmsConnectionManager =
            instance ?: BmsConnectionManager(context.applicationContext).also { instance = it }
    }
}
