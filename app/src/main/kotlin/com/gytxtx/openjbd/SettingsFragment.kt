...
class SettingsFragment : Fragment(), BmsStateStore.Listener {
    ...
    private lateinit var connectionManager: BmsConnectionManager
    ...
    private fun renderSettingsRows() {
        ...
        for (i in 0 until settingsAdapter.count) {
            ...
            row.setOnClickListener {
                ...
                if (settingSwitch != null && settingSwitch.visibility == View.VISIBLE) {
    ...
    private fun settingsGroupParent(position: Int): LinearLayout {
        return when {
            position <= 3 -> interfaceSettingsList
            position == 4 -> deviceSettingsList
            else -> otherSettingsList
        }
    }
    ...
    private fun showSettingMenu(position: Int) {
        when (position) {
            0 -> showMenu(
                ...
                AppSettings.themeValue(requireContext()),
                arrayOf(
                    getString(R.string.setting_dark_theme_system),
                    getString(R.string.setting_dark_theme_always_on),
                    getString(R.string.setting_dark_theme_always_off)
                ),
            ...
            1 -> showMenu(
                settingsRow(position),
                AppSettings.languageValue(requireContext()),
                arrayOf(
                    getString(R.string.setting_language_system),
                    getString(R.string.setting_language_zh),
            ...
            2 -> showMenu(
                settingsRow(position),
                settings().getString(AppSettings.PREF_TEMP_UNIT, AppSettings.VALUE_C) ?: AppSettings.VALUE_C,
                arrayOf(
                    getString(R.string.setting_temp_celsius),
                    getString(R.string.setting_temp_fahrenheit)
                ),
            ...
            3 -> showMenu(
                settingsRow(position),
                AppSettings.refreshIntervalValue(requireContext()),
                arrayOf(
                    getString(R.string.setting_refresh_1s),
                    getString(R.string.setting_refresh_2s),
                    getString(R.string.setting_refresh_5s),
                    getString(R.string.setting_refresh_10s)
                ),
                arrayOf(
                    AppSettings.VALUE_REFRESH_1S,
                    AppSettings.VALUE_REFRESH_2S,
                    AppSettings.VALUE_REFRESH_5S,
                    AppSettings.VALUE_REFRESH_10S
                )
            ) { value ->
                AppSettings.setRefreshIntervalMs(requireContext(), value.toLong())
                renderSettingsRows()
                if (connected) {
    ...
    private fun configureAutoReconnect() {
        val prefs = settings()
        val enabled = AppSettings.autoConnectEnabled(requireContext())
    ...
    private fun themeLabel(value: String): String = when (value) {
        AppSettings.VALUE_DARK -> getString(R.string.setting_dark_theme_always_on)
        AppSettings.VALUE_LIGHT -> getString(R.string.setting_dark_theme_always_off)
        else -> getString(R.string.setting_dark_theme_system)
    }

    private fun languageLabel(value: String): String = when (value) {
        AppSettings.VALUE_ZH -> getString(R.string.setting_language_zh)
        AppSettings.VALUE_EN -> getString(R.string.setting_language_en)
        else -> getString(R.string.setting_language_system)
    }

    private fun tempUnitLabel(value: String): String =
        if (AppSettings.VALUE_F == value) getString(R.string.setting_temp_fahrenheit)
        else getString(R.string.setting_temp_celsius)

    private fun refreshIntervalLabel(value: String): String = when (value) {
        AppSettings.VALUE_REFRESH_1S -> getString(R.string.setting_refresh_1s)
        AppSettings.VALUE_REFRESH_5S -> getString(R.string.setting_refresh_5s)
        AppSettings.VALUE_REFRESH_10S -> getString(R.string.setting_refresh_10s)
        else -> getString(R.string.setting_refresh_2s)
    }
    ...
    private inner class SettingsAdapter : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            ...
            val settingSwitch = row.findViewById<SwitchMaterial>(R.id.switch_setting_action)
            ...
            when (position) {
                0 -> {
                    ...
                    icon.contentDescription = getString(R.string.setting_dark_theme)
                    title.setText(R.string.setting_dark_theme)
                    subtitle.text = themeLabel(AppSettings.themeValue(requireContext()))
                }
                1 -> {
                    icon.setImageResource(R.drawable.ic_language_24)
                    icon.contentDescription = getString(R.string.setting_language)
                    title.setText(R.string.setting_language)
                    subtitle.text = languageLabel(AppSettings.languageValue(requireContext()))
                }
                2 -> {
                    icon.setImageResource(R.drawable.ic_thermostat_24)
                    icon.contentDescription = getString(R.string.setting_temperature_unit)
                    title.setText(R.string.setting_temperature_unit)
                    subtitle.text = tempUnitLabel(
                        prefs.getString(AppSettings.PREF_TEMP_UNIT, AppSettings.VALUE_C)
                            ?: AppSettings.VALUE_C
                    )
                }
                3, 4 -> {
                    ...
                    subtitle.text = refreshIntervalLabel(AppSettings.refreshIntervalValue(requireContext()))
                    if (position == 4) {
                        ...
                        icon.contentDescription = getString(R.string.setting_auto_connect)
                        title.setText(R.string.setting_auto_connect)
                        subtitle.text = autoConnectLabel(autoConnectEnabled)
                        settingSwitch.isChecked = autoConnectEnabled
                        settingSwitch.setOnCheckedChangeListener { _, checked ->
                            subtitle.text = autoConnectLabel(checked)
                            setAutoConnectEnabled(checked, false)
                        }
                        settingSwitch.visibility = View.VISIBLE
                    } else {
                        icon.contentDescription = getString(R.string.setting_refresh_interval)
                    }
                }
                else -> {
                    icon.setImageResource(R.drawable.ic_info_24)
                    icon.contentDescription = getString(R.string.setting_about)
